...
\caption{Vertical 2-piece height board}\label{fig:unnamed-chunk-13}
\end{figure}
...
\caption{Recumbent 2-piece height board}\label{fig:unnamed-chunk-14}
\end{figure}
...
position with the child lying down. For children 24 months and older,
height should be measured i.e., height board on the vertical position
...
\caption{Steps in measuring height}\label{fig:unnamed-chunk-16}
\end{figure}
...
\item
  ...
  device on the ground (\texttt{arrow\ 1}) and kneel with both knees on
  the right side of the child (\texttt{arrow\ 2}).
\item
  \textbf{Measurer} should kneel on their right knee only, for maximum
  mobility, on the child's left side (\texttt{arrow\ 3}).
\item
  ...
  shins (\texttt{arrow\ 4}) with left hand on the child's knees
  (\texttt{arrow\ 5}) and push against the board making sure the child's
  legs are straight and the heels and calves are against the board
  (\texttt{arrows\ 6} and \texttt{7}). The \textbf{assistant} then
...
\item
  ...
  is level with the ground (\texttt{arrow\ 8}). \textbf{Measurer} places
  open left hand on the child's chin and gradually closes hand
  (\texttt{arrow\ 9}) taking care that the child's mouth or ears are not
  covered. The \textbf{measurer} makes sure the shoulders are level
  (\texttt{arrow\ 10}), the hands are at the child's side
  (\texttt{arrow\ 11}), and the head, shoulder blades and buttocks are
  against the board (\texttt{arrows\ 12},\texttt{13} and \texttt{14}).
  ...
  child's hair (\texttt{arrow\ 15}).
\item
  \textbf{Measurer} and \textbf{Assistant} should check the child's
  position (\texttt{arrow\ 1-15}) and repeating any steps necessary.
\item
